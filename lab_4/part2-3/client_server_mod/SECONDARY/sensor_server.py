...
class Sensor_server:
    def __init__(self, host, port):
        ...
        """
        Setting up the actual server specfics using socket library
        """
        ...
        """
        The socket is all set up and ready to start listening for connections.
        """
        sock.listen()
    ...
    def run_listener(self):
        ...
        # While _no_packet has been dected, keep running the event listening loop 
    ...
    def service_connection(self, key:selectors.SelectorKey, mask):
        ...
        if mask & selectors.EVENT_WRITE:
            ...
            if data.outb:
                """
                Once we have data in the data out buffer we stop the connection to the device and return the data
                """
    ...
    def packet_post_encapsulator(self):
        ...
        # FIX ME: Add polling data here 

        data_str = self.manager.read_line() # manager set in init
        data_str = self.update_curr_line() # moves line down

        polling_list = data_str.split(",")
