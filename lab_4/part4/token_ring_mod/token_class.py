...
class token_server:
    def __init__(self, host, port, first_device):
        """
        Initialization of the server class.
        """
        slogger.info(f"__init__: starting...")
        # Setting our IP, port, and socket selector
        self.sel = selectors.DefaultSelector()
        self._host = host
        self._port = port
        self._no_packet = True
        self._emergency_not_sent = True
        
        # Binding socket to our address and port
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        sock.bind((self._host, self._port))

        # Setting up ability to use socket and allow multiple connections 
        sock.listen()
        sock.setblocking(False)
        self.sel.register(sock, selectors.EVENT_READ, data=None)
        
        
        # Setting up internal classes and variables for sql stuff
        self._poll = file_manager() # Used for getting sensor data 
        self._sql = None 
        self._emergency = None
        self._tables = None
        # The first token device is always the internet connected device, 
        # and should be the only one with SQL and Flask handlers
        if first_device:
            self._sql = mySQL()
            self._emergency = flask_handler()
            self._tables = ["sensor_readings1", "sensor_readings2", "sensor_readings3"]
    ...
    def packet_adder(self, packet, seq_number):
        ...
        poll_data = self._poll.read_line()
    ...
    def seq_handler(self, packet, seq_number):
        ...
        if int(seq_number) < int(TOTAL_HOSTS):
            ...
            slogger.info(f"seq_handler: packet is: {packet}")
            # TODO: Add code here to send packet to database (eth0)
            # NOTE: editting packet_splitter function resulted in a NoneType bug.
            all_data = packet.split("/") # splitting packet = token/20.8,47.9,0.0,38.2/20.8,47.9,0.0,38.2/....seq#3
            slogger.debug(f"seq_handler: list_data is: {all_data}")
            all_data.pop(0) #remove the token header from the list split
            slogger.debug(f"seq_handler: token removed list_data is: {all_data}")

            # Prepare the data list packet we are going to send to the sql server
            all_data = self.prepare_for_sql(all_data)
            
            # Send each devices sensor data to each table in the sql server
            for i, table_name in enumerate(self._tables):
                # TODO: FIX IT SENDNING ONLY poll_data should send list_data
                slogger.debug(f"seq_handler: inputting into table {table_name} with {[all_data[i]]}")
                self._sql.insert_update(table_name, all_data[i])

            # Start the token ring again (ad-hoc)
    ...
    def prepare_for_sql(self, all_data):
        """
        Turns a list of strings (each string is one devices polling data) into a list of lists, 
        removing the unnecessary token header seq number and adding wildfire risk data. 
        """
        ## Find which element in the list has the seq number, should be the last element always
        seq_index  = len(all_data) - 1  

        # For loop for creating a list of lists of data for each device, plus finding their wildfire risks and adding them
        slogger.debug(f"prepare_for_sql STARTING FOR LOOP #1")
        for i,item in enumerate(all_data): 
            list_data = item.split(",") #split the data str the deliminter "," into a list
            slogger.debug(f"prepare_for_sql: FOR LOOP item is: {item}")
            # Check if we are the last data str so we can remove the sequence number 
            if i == seq_index:
                slogger.debug(f"    removing sequence number")
                list_data.pop() # removing the seq number
                slogger.debug(f"    list_data is {list_data}") 
            # Find the wildfire risk percent and risk stage 
            wildfire_percent = calc_wildfire_percent_risk(list_data[0], list_data[1], list_data[2])
            wildfire_stage   = calc_wildfire_risk_stage(wildfire_percent)
            
            #Check if we need to send the emergency packet because the chance of wildfires is high
            if self._emergency_not_sent:
                if self.check_emergency(wildfire_percent): 
                    slogger.debug(f"PRIMARY:main: EMERGENCY ERMGNERNCY SDKJSDKJASOIASDJOI")
                    self._emergency_not_sent = False
                    send_emergency(self._emergency)
            
            # Append the wildfire data to the list 
            list_data.append(wildfire_stage)
            list_data.append(wildfire_percent)
            slogger.debug(f"prepare_for_sql: list_data w/wildfire: {list_data}")
            
            # Replace the current all_data index with the improved internal list of data
            all_data[i] = list_data
        slogger.debug(f"prepare_for_sql ENDING FOR LOOP")
        slogger.info(f"prepare_for_sql: all_data is: {all_data}")
        return all_data

    @classmethod
    def check_emergency(cls, percent): 
        """
